...
// ─── AUTH: téléphone chiffres uniquement ───
...
['regPhone', 'setPhone'].forEach(id => {
  const el = document.getElementById(id);
  if (el) el.addEventListener('input', () => sanitizePhoneInput(el));
});
...
// Garde le champ "Retour" cohérent avec le champ "Départ" :
// - empêche de choisir une date de retour antérieure au départ
// - si le départ est repoussé après la date de retour déjà choisie, on ajuste le retour
function syncHeroDates() {
  const depEl = document.getElementById('heroDepart');
  const retEl = document.getElementById('heroRetour');
  const errEl = document.getElementById('heroDatesError');
...
function launchSearch() {
  ...
  // Validation : la date de retour ne peut pas précéder la date de départ
  if (searchDepart && searchRetour && searchRetour < searchDepart) {
    showMsg('heroDatesError', 'La date de retour doit être après la date de départ.', 'error');
    return;
  }
  const heroErr = document.getElementById('heroDatesError');
  if (heroErr) heroErr.classList.remove('show');
